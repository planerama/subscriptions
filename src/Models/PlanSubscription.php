...
class PlanSubscription extends Model
{
     ...
     * @return bool
     */
    public function canUseFeatureByName(string $name): bool
    {
        $featureValue = $this->getFeatureValueByName($name);
        $feature = $this->getFeatureByName($name);
        $usage = $this->usage()->byFeature($feature)->first();
        ...
        // Check for available uses
        return $this->getFeatureRemainingsByName($name) > 0;
    }
     ...
     * @return int
     */
    public function getFeatureUsageByName(string $name): int
    {
        $feature = $this->plan
            ->features()
            ->where("name->" . config("app.locale"), $name)
            ->first();
        $usage = $this->usage()->byFeatureSlug($feature->slug)->first();
        return (!$usage || $usage->expired()) ? 0 : $usage->used;
    }
     ...
     * @return int
     */
    public function getFeatureRemainingsByName(string $name): int
    {
        return $this->getFeatureValueByName($name) - $this->getFeatureUsageByName($name);
    }
     ...
     * @return mixed
     */
    public function getFeatureValueByName(string $name)
    {
        $locale = config("app.locale");
        $feature = $this->plan->features()->where("name->$locale", $name)->first();
        $result = $feature->value ?? null;
        return $result;
    }
    ...
    {
        $feature = $this->plan->features()->where('name->en', $name)->first();
